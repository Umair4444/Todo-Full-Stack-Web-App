...
def test_get_todo_by_id(client: TestClient, session: Session):
    ...
    response = client.get(f"/api/todos/{todo.id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_todo(client: TestClient, session: Session):
    ...
    response = client.put(f"/api/todos/{todo.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_delete_todo(client: TestClient, session: Session):
    ...
    # Delete the todo
    response = client.delete(f"/api/todos/{todo.id}", headers=headers)
    assert response.status_code == 200
...
def test_bulk_delete_todos(client: TestClient, session: Session):
    ...
    response = client.post("/api/todos/bulk-delete", json={"todo_ids": todo_ids})
    assert response.status_code == 200
...
def test_bulk_delete_partial_success(client: TestClient, session: Session):
    ...
    response = client.post("/api/todos/bulk-delete", json={"todo_ids": invalid_ids})
    assert response.status_code == 200
...
def test_toggle_completion(client: TestClient, session: Session):
    ...
    response = client.patch(f"/api/todos/{todo.id}/toggle-completion")
    assert response.status_code == 200
...
def test_toggle_completion_not_found(client: TestClient, session: Session):
    ...
    response = client.patch("/api/todos/99999/toggle-completion")
    assert response.status_code == 404
    data = response.json()
...
def test_pagination(client: TestClient, session: Session):
    ...
    response = client.get("/api/todos?offset=0&limit=10")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 10

    response = client.get("/api/todos?offset=10&limit=10")
    assert response.status_code == 200
    data = response.json()
